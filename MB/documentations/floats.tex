...
mit Hilfe von \lstinline$includegraphics$ in das Dokument eingefügt. Das hat den Vorteil, dass man
den Suffix, also den Dateityp, bei der Angabe des Namens weglassen kann. In den meisten
Fällen, bis auf ganz alte Abbildungen, die aus den Quellen für  \cite{brill:01} stammen,
gibt es für Grafiken eine Version im \datei{png}- oder im \datei{eps}-Format. Damit wird sichergestellt,
dass die Erstellung eines \pdf{}-Dokuments mit Hilfe von \begriff{dvips} oder mit \begriff{pdflatex}
möglich ist. Stand Juli 2018 wurde auf PDF\LaTeX{} umgestellt, so dass es für neue Abbildung ausreicht
...
so in das Dokument eingefügt:

\begin{lstlisting}{}
\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{\imagePath/tugboat}
\caption{\label{tugboat}Ein schönes
...
das Abbildungen enthält, die allgemein verwendet werden, gibt es Verzeichnisse wie
...
Die deutsche Übersetzung dafür ist "`Sonstiges"'.}

\tip{
Eine mögliche Fehlerquelle, die noch nicht überall beseitigt wurde ist die Tatsache, dass Verzeichnis-Namen
in der Vergangenheit nicht eindeutig nach Groß- und Kleinschreibung vergeben wurden. Das ist
unter \windows{} kein Problem, führt aber bei Verwendung auf \linux{} zu Problemen und muss sukzessive
korrigiert werden!
}
...
\end{figure}
Neben Abbildungen, die mit EPS-Grafiken oder Bitmaps\randnotiz{Koordinaten-\\systeme} erstellt werden bietet \LaTeX{} die Möglichkeit mit
...
\end{center}
\end{lstlisting}
Die Abbildung \ref{floats:polargrid} erhält man mit den folgenden Zeilen:
\begin{lstlisting}{}
\renewcommand{\runitdivisions}{2}
\setlength{\unitlength}{0.75cm}
\renewcommand{\gridcolor}{magenta}
...
voraus. Insbesondere das Package \emph{calculator} bietet die Möglichkeit,
...
%
\section{Tabellen}
\begriff{Tabellen} werden mit \datei{table} eingefügt und haben immer eine Beschriftung, um
im Text darauf Bezug zu nehmen. Die Beschriftung wird als Tabellen-Überschrift
eingefügt, also oberhalb der Tabelle. Das stammt aus den Projekten mit dem Hanser-Verlag
und wurde seitdem beibehalten. Bei den Büchern wurde die Quelle für jede Tabelle
...
dass die Tabelle nur an einer Stelle steht und dass Korrekturen so immer für alle Dokumente
wirken, in denen die Tabelle verwendet wird.

\begin{table}[ht]
\centering
\caption{\label{tabelle}Ein Beispiel für eine Tabelle}
...
wurde mit dem folgenden Quelltext eingefügt:
...
\end{table}
\end{lstlisting}
Einträge in den Spalten sind meist linksbündig. Das muss man nicht beibehalten, aber
die Ausrichtung von Einträgen sollte immer konsistent sein.
